...
public class EduLink_Timetable {
    class public func timetable(_ rootCompletion: @escaping completionHandler) {
        ...
        let body = "{\"jsonrpc\":\"2.0\",\"method\":\"EduLink.Timetable\",\"params\":{\"date\":\"\(date())\",\"learner_id\":\"\(EduLinkAPI.shared.authorisedUser.id!)\",\"authtoken\":\"\(EduLinkAPI.shared.authorisedUser.authToken!)\"},\"uuid\":\"\(UUID.uuid)\",\"id\":\"1\"}"
